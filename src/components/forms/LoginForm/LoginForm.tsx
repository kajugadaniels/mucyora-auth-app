...
export function LoginForm() {
  ...
  useEffect(() => {
    const completionReference = new URLSearchParams(
    ...
    void authGateway
      .consumeIdentityCompletion(completionReference)
      .catch((caught) => {
        if (isAuthGatewayError(caught)) setError(caught.message);
      });
  }, []);
  const submit = async (values: LoginValues) => {
    ...
    try {
      ...
      if (!result) return;
      resetField("password");
      if (result.sessionLevel === "LIMITED" || !result.identityVerified)
        return router.replace("/verification-required");
      window.location.assign(
        process.env.NEXT_PUBLIC_MUCYORA_APP_ORIGIN || "http://localhost:4001",
      );
    } catch (caught) {
      if (!isAuthGatewayError(caught)) throw caught;
      setError(caught.message);
      if (caught.field === "email" || caught.field === "password")
        setFieldError(caught.field, { message: caught.message });
    }
  };
  return (
    <form className={styles.form} onSubmit={handleSubmit(submit)}>
      {error && (
        <Alert variant="error" title="Sign-in request not completed">
          {error}
        </Alert>
      )}
      <div className={styles.fields}>
        <Input
          ...
          inputMode="email"
          autoComplete="email"
          placeholder="alice.mukamana@example.rw"
